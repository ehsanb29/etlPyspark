...
def start_spark():
    ...
    # set spark config to default setting
    # we can set spark parameters in SPARK_HOME/conf/ or send parameters to spark-submit command
    conf = SparkConf()
...
def extract_data(spark, ds_path):
    ...
    print("1. Parse each evidence object to extract the `diseaseId`, `targetId`, and `score` fields.")
    print("Read evidence, targets and diseases datasets from parquet files ...")
...
def transform_data(spark, rdd_eva, df_targ, df_dis):
    ...
    :param spark:
    :param rdd_eva: evidence rdd
    :param df_targ: targets dataframe
    :param df_dis: disease dataframe
    :return:
    """
    ...
    # rdd_eva = rdd_eva.repartition(x) # to improve performance,
    # the number of partitions (x) is set based on the volume of data and cluster resources

    print(f"-> {rdd_eva.getNumPartitions()=}")
    ...
    df_result = df_dis.alias("diseases").join(df_eva.alias("eva"), df_dis.id == df_eva.diseaseid).\
    ...
    df_result = df_result.orderBy(df_result.median_score.asc())
    df_result.show()
...
def load_data(df_result, out_path):
    """
    :param df_result: final datafram to write in JSON file
...
def get_args(argv):
    ...
    try:
        ...
        # opts, args = getopt.getopt(argv, "i:o:", ["output="])
    except getopt.GetoptError:
        print('test.py -i <input_data_path> -o <output_file_path>')
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-i", "--input"):
            data_path = arg
